...
- The client caches the JWT in an ephemeral location (e.g. `/dev/shm/zpools.io`) so it is not persisted to disk and so it does not prompt on every command. Cache is cleared on reboot; re-login when the token expires.
- Cached tokens are stored under a configurable base directory (default: `/dev/shm/zpools.io`)—an ephemeral location, not persisted to storage—with restricted permissions. Set `ZPOOL_TOKEN_CACHE_DIR` in the rcfile or environment to override. 
- To disable JWT token caching, set it to empty in the rcfile (e.g. `ZPOOL_TOKEN_CACHE_DIR=` with no value, or `ZPOOL_TOKEN_CACHE_DIR=""`). The client will then not read or write token cache files.
...
- **SDK:** See [API reference](../python/packages/sdk/docs/api-reference.md) for programmatic PAT management.
...
- [Troubleshooting](troubleshooting.md) — Auth failures, non-interactive PAT.
